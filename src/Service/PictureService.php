...
{ 
    public function getImage(Animals $animal): ?string
    {
        $images = $animal->getImages();
        
        // Si l'animal a des images, récupérer le premier chemin d'image
        if (count($images) > 0) {
            $image = $images[0];
            return '/uploads/animals/' . $image->getFilePath();
        ...
        // Retourner un chemin d'image par défaut si aucune image n'est trouvée
        return $this->params->get('uploads_directory') . '/default.jpg';
    }
